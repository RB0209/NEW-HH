...
export default function GameBoyShell({ initialPage }){
  ...
  useEffect(()=>{
    // initialPage handling: open messages/journey if set via route param
    if (initialPage === 'messages') {
      setTimeout(()=> openInternal('messages'), 300)
    } else if (initialPage === 'journey') {
  ...
  useEffect(()=>{
    // background music
    const bg = new Audio('/sounds/bgloop.wav')
    bg.loop = true
    bg.volume = 0.18
  ...
  function handleActivate(idx){
    play('select')
    const key = menuItems[idx].toLowerCase()
    if (idx === 0){
      // Movies -> external
      window.open(EXTERNAL.movies, '_blank', 'noopener')
    } else if (idx === 1){
      window.open(EXTERNAL.playlist, '_blank', 'noopener')
    } else if (idx === 2){
      // Messages internal
      navigate('/messages')
      setPanelTitle('Morning Messages')
      setPanelContent('Retro-styled Messages page — coming soon!')
      setShowPanel(true)
    } else if (idx === 3){
      navigate('/journey')
  ...
  function finishBoot(){
    setBooting(false)
    localStorage.setItem('hh_seen_boot', '1')
  }

  // if booting show BootScreen
  if (booting){
        ...
        <div className="device-shell">
          <div className="screen-frame">
            <div className="screen" role="application" aria-label="Heaven Hub GameBoy">
              ...
              <div className="scene">
                <img src="/images/mock.png" alt="mock" className="characters" />
              </div>
          ...
          <div className="controls">
            ...
            <div className="small-buttons">
              ...
              <div className="ab">
                <div className="btn a" onClick={handleAPressed} aria-label="A button">A</div>
                <div className="btn b" onClick={handleBPressed} aria-label="B button">B</div>
              </div>
            </div>
          </div>
        </div>
